...
import re
import MySQLdb
conn = MySQLdb.connect(host= "localhost",user="root",passwd="asm123",db="acesspoint_db")
class access:
	def __init__(self):
		...
		if len(foundip) == len(foundname) == len(foundpsk) == len(foundstate) == len(foundmesh_role) == len(foundtimer) == len(foundtunnel) == len(foundsec) == len(foundSW) == len(foundHW) == len(foundModel) == len(foundSerial):
			...
			for i in range(len(foundip)):
				...
				cursor.execute("""insert into accesspoint(IPv4Address,Name,State,Tunnel,Sec_Mode,Mesh_Role,PSK,Timer,HW_Version,SW_Version,Model,Serial_number) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s');""" %(foundip[i],foundname[i],foundpsk[i],foundstate[i],foundmesh_role[i],foundtimer[i],foundtunnel[i],foundsec[15:18],foundSW[19:23],foundHW[i],foundModel[i],foundSerial[9:21]))
